...
resource "yandex_compute_instance" "vm-1" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "cd /opt/tomcat/ && sudo sh -c 'chmod +x /opt/tomcat/bin/*.sh' && mkdir -p /opt/tomcat/app && cd /opt/tomcat/app && sudo git clone https://github.com/boxfuse/boxfuse-sample-java-war-hello.git",
      "cd /opt/tomcat/appboxfuse-sample-java-war-hello && sudo mvn package",
